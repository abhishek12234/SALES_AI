...
class AICoachingDAL:
    ...
    async def generate_coaching_feedback(self, user_id: str, session_id: str) -> Dict:
        ...
        try:
            ...
            coaching_prompt = f"""
You are an expert sales coach analyzing a real sales conversation. Based on the conversation below, provide specific, actionable coaching feedback of the human as user conversations not just the ai conversation.

Conversation:
{conversation_text}

Provide coaching feedback specifically focused on these new messages. Analyze:
[important] strictly respond in this format only nothing else:
...
- Brief but insightful (IMPORTANT: respond in between 15 to 20 words only for each section)

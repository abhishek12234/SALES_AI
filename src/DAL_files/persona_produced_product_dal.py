...
class PersonaProducedProductDAL:
    ...
    async def get_persona_produced_products_by_persona_id(self, persona_id: str, db_session: AsyncSession):
        result = await db_session.execute(
            select(ProducedProductCategory)
            .join(PersonaProducedProduct, ProducedProductCategory.product_id == PersonaProducedProduct.product_id)
            .where(PersonaProducedProduct.persona_id == persona_id)
        )
        return result.scalars().all() 